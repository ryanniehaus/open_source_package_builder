	  ...
	  echo "working folder: $tempFolder.working"
		...
		then		  
		  ...
		  then
				...
				curl -1 -X POST -u "ryanniehaus:$GITHUB_PERSONAL_ACCESS_TOKEN" --data-binary "@$archiveFileName"  --header "Accept: application/vnd.github.v3+json" --header "Content-Type: application/gzip" "$uploadBaseURL?name=$archiveFileName&label=archive%20of%20source%20used%20to%20create%20build%20for%20$projectName%20v$projectVersion"
				curlRetval="$?"
				if [ "$curlRetval" == "0" ]
				then
				  archiveSuccess=1
